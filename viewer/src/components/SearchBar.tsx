    ...
    <div ref={searchRef} className="relative w-96">
      <div className="relative">
        <input
          ...
          className="w-full px-4 py-2 text-sm border rounded-md bg-white dark:bg-purple-950 border-gray-300 dark:border-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400"
        />
        <svg
      ...
      {isOpen && results.length > 0 && (
        <div className="absolute top-full mt-1 w-full bg-white dark:bg-purple-950 border border-gray-300 dark:border-slate-800 rounded-md shadow-lg max-h-96 overflow-y-auto">
          {results.map((result, index) => (
            <button
              key={result.path}
              onClick={() => handleSelect(result)}
              className={`w-full px-4 py-2 text-left hover:bg-gray-100 dark:hover:bg-purple-900 ${
                index === selectedIndex ? 'bg-gray-100 dark:bg-purple-900' : ''
              }`}
            >
