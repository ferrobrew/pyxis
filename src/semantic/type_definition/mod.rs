        ...
        match attribute {
            grammar::Attribute::Function(ident, items) => {
                ...
                match (ident.as_str(), exprs.as_slice()) {
                    ...
                    ("min_size", [grammar::Expr::IntLiteral(spanned)]) => {
                        min_size = Some(
                            spanned.value
                                .try_into()
                    ...
                    ("singleton", [grammar::Expr::IntLiteral(spanned)]) => {
                        singleton = Some(spanned.value.try_into().with_context(|| {
                            format!(
                    ...
                    ("align", [grammar::Expr::IntLiteral(spanned)]) => {
                        align = Some(spanned.value.try_into().with_context(|| {
        ...
        match field {
            ...
            grammar::TypeField::Vftable(functions) => {
                ...
                for attribute in attributes {
                    ...
                    let exprs = grammar::AttributeItem::extract_exprs(items);
                    if let ("size", [grammar::Expr::IntLiteral(spanned)]) =
                        (ident.as_str(), exprs.as_slice())
                    {
                        size = Some(spanned.value as usize);
                    }
                }
