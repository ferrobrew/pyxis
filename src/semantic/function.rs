...
pub fn build(
    ...
    for attribute in &function.attributes {
        ...
        match (ident.as_str(), &exprs[..]) {
            ("address", [grammar::Expr::IntLiteral(spanned)]) => {
                if is_vfunc {
                    anyhow::bail!(
                ...
                body = Some(FunctionBody::Address {
                    address: spanned.value.try_into().with_context(|| {
                        format!(
            ...
            ("calling_convention", [grammar::Expr::StringLiteral(spanned)]) => {
                let cc = &spanned.value;
