...
fn write_module(
    ...
    for func in &public_functions {
        if let Some(text) = render::render_free_function_decl(func, key, registry)? {
            if wrote_anything {
                writeln!(body)?;
            }
            body.push_str(&text);
    ...
    for ev in &sorted_externs {
        let text = render::render_extern_value_decl(ev, key, registry)?;
        if wrote_anything {
            writeln!(body)?;
        }
        body.push_str(&text);
    ...
    if needs_cpp {
        ...
        for func in &public_functions {
            if let Some(text) = render::render_free_function_definition(func, key, registry)? {
        ...
        for ev in &sorted_externs {
            let text = render::render_extern_value_definition(ev, key, registry)?;
            for line in text.lines() {
                if line.is_empty() {
                    writeln!(cpp)?;
                } else {
...
pub fn write_runtime_header(out_dir: &Path) -> Result<()> {
    ...
    writeln!(&mut out)?;
    // Calling-convention shim. We always target MSVC ABI (x86 32-bit Windows
    // via clang-cl + xwin); on non-MSVC dev hosts the macros expand to
    // nothing so the headers still compile-check during iteration.
    writeln!(&mut out, "#if defined(_MSC_VER)")?;
    writeln!(&mut out, "#  define PYXIS_CDECL __cdecl")?;
    writeln!(&mut out, "#  define PYXIS_STDCALL __stdcall")?;
    writeln!(&mut out, "#  define PYXIS_FASTCALL __fastcall")?;
    writeln!(&mut out, "#  define PYXIS_THISCALL __thiscall")?;
    writeln!(&mut out, "#  define PYXIS_VECTORCALL __vectorcall")?;
    writeln!(&mut out, "#elif (defined(__i386__) || defined(_M_IX86)) && (defined(__GNUC__) || defined(__clang__))")?;
    writeln!(&mut out, "#  define PYXIS_CDECL __attribute__((cdecl))")?;
    writeln!(&mut out, "#  define PYXIS_STDCALL __attribute__((stdcall))")?;
    writeln!(&mut out, "#  define PYXIS_FASTCALL __attribute__((fastcall))")?;
    writeln!(&mut out, "#  define PYXIS_THISCALL __attribute__((thiscall))")?;
    writeln!(&mut out, "#  define PYXIS_VECTORCALL __attribute__((sysv_abi))")?;
    writeln!(&mut out, "#else")?;
    writeln!(&mut out, "#  define PYXIS_CDECL")?;
    writeln!(&mut out, "#  define PYXIS_STDCALL")?;
    writeln!(&mut out, "#  define PYXIS_FASTCALL")?;
    writeln!(&mut out, "#  define PYXIS_THISCALL")?;
    writeln!(&mut out, "#  define PYXIS_VECTORCALL")?;
    writeln!(&mut out, "#endif")?;
    writeln!(&mut out)?;
    // Phase 2 still uses opaque atomic placeholders — Phase 3 wires up
    // `<atomic>` via #[cpp_header] on extern types.
